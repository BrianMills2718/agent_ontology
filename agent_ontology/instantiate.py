...
def generate_agent(spec, persistent_stores=False):
    ...
    w('')
    w('')
    w('def call_llm(model, system_prompt, user_message, temperature=0.7, max_tokens=8192, retries=3):')
    ...
    w('')
    w('')
    w('def call_embedding(texts, model="text-embedding-3-small", provider="openai"):')
    w('    """Generate embeddings for a list of texts. Returns list of float vectors."""')
    w('    if isinstance(texts, str):')
    w('        texts = [texts]')
    w('    if provider == "openai" or model.startswith("text-embedding"):')
    w('        from openai import OpenAI')
    w('        client = OpenAI()')
    w('        response = client.embeddings.create(model=model, input=texts)')
    w('        return [item.embedding for item in response.data]')
    w('    elif provider == "google" or model.startswith("models/"):')
    w('        from google import genai')
    w('        client = genai.Client(api_key=os.environ.get("GEMINI_API_KEY", ""))')
    w('        result = client.models.embed_content(model=model or "models/text-embedding-004", contents=texts)')
    w('        return result.embeddings')
    w('    elif provider == "huggingface" or provider == "local":')
    w('        try:')
    w('            from sentence_transformers import SentenceTransformer')
    w('            _emb_model = SentenceTransformer(model or "all-MiniLM-L6-v2")')
    w('            return _emb_model.encode(texts).tolist()')
    w('        except ImportError:')
    w('            print("    [WARN] sentence-transformers not installed, returning empty embeddings")')
    w('            return [[] for _ in texts]')
    w('    else:')
    w('        from openai import OpenAI')
    w('        client = OpenAI()')
    w('        response = client.embeddings.create(model=model, input=texts)')
    ...
    if stores:
        ...
        if has_vector:
            ...
            w('    """Vector store backed by ChromaDB with real embedding support."""')
            w('    def __init__(self, name, embedding_model=None, embedding_provider=None):')
            w('        self._fallback = []')
            w('        self._collection = None')
            w('        self._embedding_model = embedding_model')
            w('        self._embedding_provider = embedding_provider')
            w('        if _USE_CHROMA:')
            w('            self._collection = _chroma_client.get_or_create_collection(name)')
            w('            print(f"    [ChromaDB] collection \'{name}\' ready")')
            w('            if embedding_model:')
            w('                print(f"    [Embeddings] using {embedding_model} ({embedding_provider or \'auto\'})")')
            w('        else:')
            w('            print(f"    [VectorStore] using in-memory fallback (pip install chromadb for semantic search)")')
            w('')
            w('    def _embed(self, texts):')
            w('        """Generate embeddings if an embedding model is configured."""')
            w('        if not self._embedding_model:')
            w('            return None')
            w('        try:')
            w('            return call_embedding(texts, model=self._embedding_model, provider=self._embedding_provider or "openai")')
            w('        except Exception as e:')
            w('            print(f"    [WARN] embedding failed: {e}, falling back to ChromaDB default")')
            w('            return None')
            w('')
            ...
            w('            if query:')
            w('                kwargs = {"n_results": min(top_k, max(self._collection.count(), 1))}')
            w('                emb = self._embed([query])')
            w('                if emb:')
            w('                    kwargs["query_embeddings"] = emb')
            w('                else:')
            w('                    kwargs["query_texts"] = [query]')
            w('                results = self._collection.query(**kwargs)')
            ...
            w('            kwargs = {"documents": [text], "ids": [doc_id]}')
            w('            if clean_meta:')
            w('                kwargs["metadatas"] = [clean_meta]')
            w('            emb = self._embed([text])')
            w('            if emb:')
            w('                kwargs["embeddings"] = emb')
            w('            self._collection.add(**kwargs)')
            w('        else:')
        ...
        for s in stores:
            ...
            if stype == "vector":
                emb_cfg = s.get("embedding_config", {})
                emb_model = emb_cfg.get("model", "")
                emb_provider = emb_cfg.get("provider", "")
                if emb_model:
                    w(f'        self._store = _ChromaVectorStore("{_safe_name(sid)}", embedding_model="{emb_model}", embedding_provider="{emb_provider or "openai"}")')
                else:
                    w(f'        self._store = _ChromaVectorStore("{_safe_name(sid)}")')
            elif stype in ("knowledge_graph", "ontology"):
                w(f'        self._triples = []')
                w(f'        try:')
                w(f'            import networkx as nx')
                w(f'            self._graph = nx.DiGraph()')
                w(f'            self._has_nx = True')
                w(f'        except ImportError:')
                w(f'            self._graph = None')
                w(f'            self._has_nx = False')
                w(f'        self.data = {{}}')
            elif stype in ("logic_program", "pddl_domain"):
                w(f'        self.data = {{}}')
                w(f'        self._rules = []')
            elif stype == "file":
                if persistent_stores:
            ...
            if stype == "vector":
                ...
                w(f'        if key is None:')
                w(f'            return self._triples')
                w(f'        # Search triples where key matches subject, predicate, or object')
                w(f'        matches = [t for t in self._triples if key in str(t[0]) or key in str(t[1]) or key in str(t[2])]')
                w(f'        if self._has_nx and not matches:')
                w(f'            if self._graph.has_node(key):')
                w(f'                matches = [(key, d.get("predicate","related"), n) for n, d in self._graph[key].items()]')
                w(f'        return matches')
            elif stype in ("logic_program", "pddl_domain"):
                w(f'        if key is None:')
                w(f'            return self._rules')
                w(f'        return [r for r in self._rules if key in str(r)]')
            elif stype == "queue":
            ...
            if stype == "vector":
                ...
                w(f'        if isinstance(value, tuple) and len(value) == 3:')
                w(f'            self._triples.append(value)')
                w(f'            if self._has_nx:')
                w(f'                self._graph.add_edge(value[0], value[2], predicate=value[1])')
                w(f'        elif isinstance(value, dict):')
                w(f'            t = (value.get("subject",""), value.get("predicate",""), value.get("object",""))')
                w(f'            self._triples.append(t)')
                w(f'            if self._has_nx:')
                w(f'                self._graph.add_edge(t[0], t[2], predicate=t[1])')
                w(f'        else:')
                w(f'            self._triples.append(("_", "_", str(value)))')
            elif stype in ("logic_program", "pddl_domain"):
                w(f'        self._rules.append(value)')
            elif stype == "queue":
    ...
    if agents:
        ...
        for a in agents:
            ...
            max_tok = config.get("max_tokens", 8192)
...
def generate_langgraph_agent(spec, persistent_stores=False):
    ...
    w('')
    w('')
    w('def _get_chat_model(model, temperature=0.7, max_tokens=8192):')
    ...
    w('')
    w('def call_embedding(texts, model="text-embedding-3-small", provider="openai"):')
    w('    """Generate embeddings for a list of texts."""')
    w('    if isinstance(texts, str):')
    w('        texts = [texts]')
    w('    if provider == "openai" or model.startswith("text-embedding"):')
    w('        from openai import OpenAI')
