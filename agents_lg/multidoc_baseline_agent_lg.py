#!/usr/bin/env python3
"""
MultiDoc Baseline — Generated by Agent Ontology Instantiation Engine (LangGraph backend)
Spec: Single-shot multi-document reasoning agent. Receives all fact cards concatenated as context plus a question, and produces an answer in one LLM call. Deliberately simple — serves as the baseline for evolution experiments on the multidoc benchmark.
...
class AgentState(TypedDict, total=False):
    """Typed state for MultiDoc Baseline"""
...
def invoke_solver(user_message, output_schema=None):
    """Document Reasoner"""
    system = """You are a document reasoning assistant. You will receive a set of fact cards (labeled sources) and a question. Read all the sources carefully, then answer the question.
Output your answer as a JSON object with a single "answer" field. The answer should be concise — a name, number, or short phrase. Do not include explanation in the answer field.
"""
    if output_schema:
    ...
    trace_call("Document Reasoner", "gemini-3-flash-preview", system, user_message, result, int((time.time()-t0)*1000))
    return result
...
def node_solve(state: AgentState) -> dict:
    """
    Solve
    Send context + question to the LLM and get an answer
    """
    print(f"  → Solve")
    updates = {}
    ...
    _cur = dict(state)
    _cur.update(updates)
    solver_input = build_input(_cur, "ReasoningInput")
    solver_msg = json.dumps(solver_input, default=str)
    solver_raw = invoke_solver(solver_msg, output_schema="ReasoningOutput")
    solver_result = parse_response(solver_raw, "ReasoningOutput")
    updates["_schema_violations"] = len(validate_output(solver_result, "ReasoningOutput"))
    updates.update(solver_result)
    updates["reasoning_output"] = solver_result
    updates["solve_result"] = solver_result
    print(f"    ← Document Reasoner: {solver_result}")
...
def build_graph():
    ...
    graph.add_edge("receive_query", "solve")
    graph.add_edge("solve", "emit_answer")
...
def run(initial_data=None):
    """MultiDoc Baseline — LangGraph execution"""
    app = build_graph()
    ...
    print(f"\n════════════════════════════════════════════════════════════")
    print(f"  MultiDoc Baseline (LangGraph)")
    print(f"  Single-shot multi-document reasoning agent. Receives all fact cards concatenated as context plus a question, and produces an answer in one LLM call. Deliberately simple — serves as the baseline for evolution experiments on the multidoc benchmark.")
    print(f"════════════════════════════════════════════════════════════\n")
