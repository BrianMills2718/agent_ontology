#!/usr/bin/env python3
"""
KB ReAct — Generated by Agent Ontology Instantiation Engine
Spec: ReAct agent that queries a fictional knowledge base using search, lookup, and calculate tools
"""
...
def invoke_reasoning_agent(user_message, output_schema=None):
    """Reasoning Agent"""
    system = """You are a ReAct agent that answers questions by querying a knowledge base.
...
Strategy:
1. First SEARCH for the entity mentioned in the question to find its ID
2. Then LOOKUP that ID to get full details (relationships, numbers, etc.)
3. If the answer requires following a relationship (e.g., founder_id, manufacturer_id, hq_city), LOOKUP the referenced ID
4. If math is needed, use CALCULATE
5. When you have the final answer, set type to "answer"
...
def tool_search_tool(input_text):
    """KB Search: Search the knowledge base by name or keyword to find entity IDs"""
    import requests
    import re as _re
...
def tool_lookup_tool(input_text):
    """KB Lookup: Look up full details of a knowledge base entity by its ID"""
    import requests
    _headers = {"User-Agent": "AgentOntology/1.0 (agent research project)"}
    try:
        resp = requests.get("https://en.wikipedia.org/w/api.php", params={
...
def run(initial_data=None):
    ...
    print(f"\n════════════════════════════════════════════════════════════")
    print(f"  KB ReAct")
    print(f"  ReAct agent that queries a fictional knowledge base using search, lookup, and calculate tools")
    print(f"════════════════════════════════════════════════════════════\n")
